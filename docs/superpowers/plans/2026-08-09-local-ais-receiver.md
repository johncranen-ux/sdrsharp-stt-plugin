...
### Task 1: The recorder and its admission rule
...
**Interfaces:**
- Consumes: `ais._vessel_cache`, `ais._callsign_cache`, `ais._cache_lock`, `ais._now()`
- Produces:
  - `ais.record(fields: dict, *, source: str, observed_at: float | None = None) -> None`
  - `ais._mmsi_index: dict[str, dict]`
  - `ais._pending: dict[str, dict]`
  - `ais.AIS_LOCAL_MAX_KM: float`
  - `ais._km_from_maas(lat: float, lon: float) -> float`

`fields` is a partial observation. Recognised keys: `mmsi` (str, required), `name`,
`callsign`, `type`, `imo`, `length`, `beam`, `draught`, `destination`, `latitude`,
`longitude`, `sog`, `cog`, `heading`. Any subset may be present.

**Naming and admission are ONE rule, which is why they are one task.** Raw AIS splits a
vessel across messages — types 1/2/3 carry MMSI and position with no name, type 5 carries
the name with no position — so neither alone can decide whether a vessel belongs in the
cache. Observations accumulate in `_pending` keyed by MMSI until the vessel has a name and,
when the radius filter is on, a position inside it.

**The radius gates admission only, not later updates.** Once a vessel is in the cache its
subsequent positions apply even if it moves outside; rejecting them would freeze a stale
fix, which is worse than tracking a vessel that has wandered.
...
Run: `py -m pytest server/tests/test_whisper_proxy.py -q -k "observation or nameless or stale or newest or no_mmsi or known_mmsi or never_erases or radius or admitted or scheveningen or static_only or filter_is_off"`
Expected: FAIL with `AttributeError: module 'stt_proxy.ais' has no attribute 'record'`

- [ ] **Step 4: Implement the recorder**

Add `import math` to the imports at the top of `ais.py` (`time` is already imported).

Add near the other config constants (beside `AIS_MAX_AGE_MIN`, line 245):
...
Add immediately before `_process_ais` (line 388):
...
### Task 2: Move aisstream onto the recorder
...
Note the radius filter from Task 1 now applies to aisstream too. Existing tests use positions inside the radius or none at all; if one fails on the filter, monkeypatch `AIS_LOCAL_MAX_KM` to `0` **in that test only** and note why.
...
### Task 3: The AIS-catcher JSON adapter
...
**Interfaces:**
- Consumes: `ais_local.parse_message` (Task 3), `ais.record` (Task 1)
- Produces:
...
### Task 5: Wiring, silence watchdog, and observability
...
**Interfaces:**
- Consumes: `ais_local.start`, `ais_local.stats` (Task 4)
...
**Spec coverage:** external AIS-catcher process (Task 5 step 7) · UDP JSON consumption (Tasks 3, 4) · provider-neutral recorder (Task 1) · MMSI index and pending positions (Task 1) · merge rules incl. newest-wins position (Task 1) · radius filter (Task 1) · aisstream onto the recorder (Task 2) · silence watchdog (Task 5) · no-`SO_REUSEADDR` bind (Task 4) · malformed JSON rate-limited (Task 4) · per-source counters on `/api/ais-cache` (Task 5) · offline tests throughout. **All spec sections have a task.**

**Out of scope, as specified:** AISHub, proxy supervision of AIS-catcher, the webapp, changes to the name matcher or resolver.

**Type consistency:** `record(fields, *, source, observed_at=None)` is used with that exact signature in Tasks 1, 2, 4. `parse_message(msg) -> dict | None` matches its use in Task 4. `stats()` returns the four keys asserted in Tasks 4 and 5. `mmsi` is a `str` everywhere — Task 3 converts AIS-catcher's integer explicitly, with a test pinning it.
